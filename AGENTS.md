...
| PyPI platform-wheel distribution (cargo-free consumer install) | `uv tool install code-reality` / `uv tool install pyrefly-producer` / `uv tool install code-reality-lsp-bridge` — wheels on PyPI (macOS arm64; v0.2.0 first release 2026-08-28); release path: `.github/workflows/release-wheels.yml` on `v*` tags via trusted publishing (three per-dist GitHub environments). One-shot use: `uvx code-reality <tool> ...` where dist name = bin name; the producer dist needs `uvx --from pyrefly-producer <bin>`. rust-analyzer stays a system dependency (`rustup component add rust-analyzer`); `lsp_status` reports missing backends as `state=unavailable` with install guidance. **Main face** — the npm embedded face (row below) is additive; the platform-package pin is force-synced to the plugin version (user adjudication 2026-08-29, superseding the lag-tolerant design): the release order is platform-first — bump workspace + npm package source, tag-publish via CI, then the plugin commit aligns version + pin + lockfile (the pin can only resolve after the npm package exists); the wrapper test enforces pin == plugin version from that commit on | ✅ |
| CC one-click full stack (npm embedded face) | `claude plugin install code-reality@code-reality-market` — the plugin's `package.json` `optionalDependencies` pull npm `code-reality-darwin-arm64` (the five bins assembled verbatim from the same-tag PyPI wheels; os/cpu-gated); CC runs `npm ci` on install and the servers spawn from `node_modules/.bin` via the wrapper chain (row above). macOS arm64 with arm64 npm only — an x64 npm (Rosetta node) skips the package and the wrapper falls through to the uv face; ZCode has no npm mechanism (uv face, README-documented; uptake waits on native CC-convention parity — the wrapper already degrades gracefully, and a spawn-time ZCode auto-install was adjudicated out 2026-08-29 in favor of waiting for native support). Release: `release-wheels.yml` npm-pack/npm-publish jobs (version guard vs workspace; single `release-npm` environment, npm trusted publishing/OIDC — tokenless) — 0.3.0 bootstrapped from the PyPI wheels (registry live); github-marketplace install verified end-to-end (npm ci in cache, root parity; both servers connected) | ✅ |
