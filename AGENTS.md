...
| Legacy CRG graph.db import (one-shot) | `code-reality graph_db import_legacy --repo <repo> [--dry-run] [--json]` — Function nodes merge onto producer symbols, the rest mint qname-keyed symbols (`treesitter-legacy`); legacy db opened read-only (oracle) | ✅ |
...
| CRG retirement readiness | engine layer READY — `ai-analysis/reports/s4-crg-retirement-readiness.md`; **consumer cutover DONE 2026-08-26**; **format ownership flip DONE 2026-08-27** (`ep-v1plus-own-graph-db.md`: `.code-reality/graph.db` self-owned schema, engine read chain self-produced; legacy `.code-review-graph/` kept as parity oracle/rollback) | ✅ |
