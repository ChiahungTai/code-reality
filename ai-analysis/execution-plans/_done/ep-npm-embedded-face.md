...
4. ~~push＋github marketplace 最後一哩~~ **DONE**（2026-08-29）——
   ...
   一致性證實**（clone→cache→npm ci 跑在 cache＝`${CLAUDE_PLUGIN_ROOT}`
   指向處；x64 npm 機器 node_modules 空＝SM-2 設計行為），真 plugin
   雙 server `Successfully connected`（93／94ms）。GH environment
   `release-npm` 於首次 tag run 前確認。
